...
class MainWindow(QMainWindow):
    ...
    def _create_toolbar(self):
        ...
        self._data_actions = [
            ...
            self._act_rm_top, self._act_header,
    ...
    def _update_status(self):
        df = self.engine.current
        if df is None:
            self._status_label.setText('Ready')
            return
        rows, cols = df.shape
        n_steps = len(self.recipe.steps)
        src = os.path.basename(self.engine.source_path)
        self._status_label.setText(
            f'{rows:,} rows  ×  {cols} columns    |    {n_steps} step(s)    |    {src}'
        )
    ...
    def _apply_style(self):
        self.setStyleSheet("""
            ...
            #pipeline_list::item:selected { background-color: #E8F0FE; }
